...
RadialLayout.prototype.createNetwork = function () {
  ...
  net.createVertex = function () {
    return new RadialVertex();
  };
  return net;
}
...
RadialLayout.prototype.doLayout = function (coll) {
  ...
  let rootvert = this.network.findVertex(this.root);
...
RadialLayout.prototype.findDistances = function (source) {
  ...
  let seen = new go.Set(go.RadialVertex);
  seen.add(source);
  ...
  let finished = new go.Set(go.RadialVertex);
  while (seen.count > 0) {
